...
module.exports = function (db) {
      ...
      db.query(
        ...
        (err, data) => {
          ...
          if (data.rows.length == 0) {
            return res.json(
              new Response({ message: "unregistered e-mail" }, false)
            );
          }
          ...
          bcrypt.compare(
            ...
            async function (err, result) {
              ...
              const { rows } = await db.query(
                `WITH updated AS (UPDATE public.users SET token = $1 WHERE id_users = $2 RETURNING *) SELECT * FROM updated LEFT JOIN outlet ON updated.id_outlet = outlet.id_outlet;`,
                [token, data.rows[0].id_users]
              );
              res.json(
                new Response({
  ...
  router.get("/logout", async function (req, res) {
    ...
    if (token && token.split(" ")[1]) {
      ...
      try {
        ...
        const { rows } = await db.query(
          `SELECT * FROM public.users WHERE id_users = ${result.userid} ORDER BY id_users ASC`
        );
        const user = rows[0];
        var tokenNow = null;
        const { data } = await db.query(
          `UPDATE public.users SET token = $1 WHERE id_users = $2 RETURNING *;`,
          [tokenNow, user.id_users]
        );
  ...
  router.get("/utama", isLoggedIn, async function (req, res) {
    try {
      ...
      db.query(
        "SELECT sum(jd.qty * (var.harga_jual_varian - var.harga_beli_varian)) AS profit FROM penjualan_detail jd LEFT JOIN varian var ON var.id_varian = jd.id_varian",
        (err, profit) => {
          if (err) return res.status(500).json(new Response(err, false));
          db.query(
            "SELECT sum(total_harga_jual) AS uangmasuk FROM penjualan",
            (err, uangMasuk) => {
              if (err) return res.status(500).json(new Response(err, false));
              db.query(
                "SELECT sum(total_harga_beli) AS uangkeluar FROM pembelian",
                (err, uangKeluar) => {
                  if (err)
                    return res.status(500).json(new Response(err, false));
                  ...
                  db.query(reqSQL, argumentSQL, (err, jual1d) => {
                    ...
                    db.query(reqSQL, argumentSQL, (err, beli1d) => {
                      if (err)
                        return res.status(500).json(new Response(err, false));
                      db.query(
                        `SELECT count(no_invoice) AS totaljual FROM penjualan`,
                        (err, totaljual) => {
                          if (err)
                            return res
                              .status(500)
                              .json(new Response(err, false));
                          db.query(
                            `SELECT count(no_invoice) AS totalbeli FROM pembelian`,
                            (err, totalbeli) => {
                              if (err)
                                return res
                                  .status(500)
                                  .json(new Response(err, false));
                              db.query(
                                `SELECT v.id_varian, v.nama_varian, v.gambar_varian, b.nama_barang, s.nama_satuan, sum(pd.qty) as qty FROM penjualan_detail pd LEFT JOIN varian v ON pd.id_varian = v.id_varian LEFT JOIN barang b ON v.id_barang = b.id_barang LEFT JOIN satuan s ON v.id_satuan = s.id_satuan GROUP BY v.id_varian, b.nama_barang, s.nama_satuan ORDER BY qty DESC LIMIT 5`,
                                (err, topproduct) => {
                                  if (err)
                                    return res
                                      .status(500)
                                      .json(new Response(err, false));
                                  db.query(
                                    `SELECT o.id_outlet, o.nama_outlet, o.alamat_outlet, o.kontak_outlet, COUNT(p.id_outlet) as total_jual FROM penjualan AS p LEFT JOIN outlet o ON p.id_outlet = o.id_outlet GROUP BY o.id_outlet ORDER BY total_jual DESC LIMIT 5`,
                                    (err, topoutlet) => {
                                      if (err)
                                        return res
                                          .status(500)
                                      ...
                                      res.json(
                                        new Response({
                                          ...
                                          uangMasuk:
                                            uangMasuk.rows[0].uangmasuk,
                                          uangKeluar:
                                            uangKeluar.rows[0].uangkeluar,
                                          jualHariIni: jual1d.rows,
                                          beliHariIni: beli1d.rows,
                                          totaljual:
                                            totaljual.rows[0].totaljual,
                                          totalbeli:
                                            totalbeli.rows[0].totalbeli,
